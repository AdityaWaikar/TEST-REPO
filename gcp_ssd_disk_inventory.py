...
def get_vm_disk_data(project_id):
    """Collect VM and SSD persistent disk data from GCP project"""
        ...
        try:
            ...
            for instance in instances:
                ...
                for disk in instance.get('disks', []):
                    ...
                    # Check if it's SSD persistent disk
                    if disk_type == 'pd-ssd':
...
def create_excel_report(data, output_file="gcp_ssd_disk_inventory.xlsx"):
    """Create an Excel report with the VM and disk data"""
    if not data:
        print("No VMs with SSD persistent disks found.")
        return False
    ...
    print(f"Total VMs with SSD persistent disks: {len(vm_to_row)}")
    return True

def main():
    project_id = 'cas-prod-env'  # Use your project ID here
    
    print(f"Collecting VM and SSD persistent disk data for project: {project_id}")
    vm_disk_data = get_vm_disk_data(project_id)
    
    if vm_disk_data:
        print(f"Found {len(vm_disk_data)} instances of SSD persistent disks")
